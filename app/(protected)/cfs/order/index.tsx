...
export default function CfsOrdersListPage() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    const lowerQurey = searchQuery.toLowerCase().trim();
    return list.filter((o) => o.id.toLowerCase().includes(lowerQurey));
  ...
  if (loading) {
    return (
      <LoadingView LoadingText="Loading Orders..." />
    )
  }
  ...
  return (
    <>
      <Stack.Screen
        options={{
          headerTitle: selectionMode ? `${selectedIds.length} selecte` : "Order",
          headerRight: selectionMode
            ...
            : () => (
              <Button variant="default" onPress={() => router.push('/(protected)/cfs/order/create')}>
                <Text>New Order</Text>
              </Button>
            ),
          headerLeft: () => (
                ...
                <CardContent>
                  <Text className="text-muted-foreground">
                    {searchQuery.trim()
                      ...
                      : "You haven't created any CFS orders yet."}
                  </Text>
                </CardContent>
              </Card>
            ) : (
                ...
                const renderRightActions = () => (
                  <View className="flex-row h-full rounded-xl overflow-hidden">
                    <Pressable
                      ...
                      onPress={() => {
                        router.push({
                          pathname: "/(protected)/cfs/order/edit/[orderId]",
                            ...
                            { text: "Cancel", style: "cancel" },
                            {
                              ...
                              onPress: async () => {
                                try {
                                  ...
                                  const res = await deleteCfsOrdersBulk([order.id]);
                                  if (!res.success) {
                                    Alert.alert("Error", res.message);
                                    return;
                                  }
