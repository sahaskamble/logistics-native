...
import { get3plOrderById } from "@/lib/actions/3pl/fetch";
...
export default function ThreePlOrderViewPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const res = await get3plOrderById(orderId);
  ...
  const containers = Array.isArray((order as any).containers) ? ((order as any).containers as string[]) : [];

  return (
    <>
      <Stack.Screen
        options={{
          headerTitle: `Order #${(orderId as string).slice(0, 8)}`,
          headerLeft: () => (
          ...
          headerRight: () => (
            <Button
              ...
              onPress={() =>
                router.push({
                  pathname: "/(protected)/orders/3pl/order/edit/[orderId]",
