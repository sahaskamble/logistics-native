...
export default function WarehousePricingRequestCreatePage() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await pb.collection("warehouse_pricing_request").create({
        user: user.user.id,
      ...
      try {
        await createNotificationForCurrentUser({
          title: "Warehouse Pricing Request Submitted",
          description: `Your pricing request for ${provider?.title || "warehouse provider"} has been submitted.`,
          type: "alert",
        });
      } catch (_) {}
      Alert.alert("Success", "Pricing request submitted successfully.", [
        { text: "OK", onPress: () => router.replace("/(protected)/warehouse/pricing-request") },
      ]);
    } catch (err: any) {
  ...
  return (
    <>
      <Stack.Screen
        options={{
          headerTitle: "New Pricing Request",
          headerLeft: () => (
        ...
        <View className="p-4 gap-4">
          {provider && (
            <View className="p-4 bg-muted rounded-lg">
              ...
              {provider.description && <Text className="text-sm text-muted-foreground mt-1">{provider.description}</Text>}
            </View>
          )}
          <View className="gap-4">
            <View>
              ...
              <Select value={containerType} onValueChange={setContainerType}>
                ...
                <SelectContent>
                  {containerTypes.map((o) => <SelectItem key={o.value} value={o.value} label={o.label}>{o.label}</SelectItem>)}
                </SelectContent>
              </Select>
            </View>
            <View>
              ...
              <Select value={delayType} onValueChange={setDelayType}>
                <SelectTrigger><SelectValue placeholder="Select type" /></SelectTrigger>
                <SelectContent>
                  {delayTypes.map((o) => <SelectItem key={o.value} value={o.value} label={o.label}>{o.label}</SelectItem>)}
                </SelectContent>
              </Select>
            </View>
            <View>
              <Label>Preferable Rate</Label>
              <Input value={preferableRate} onChangeText={setPreferableRate} placeholder="Rate" keyboardType="numeric" />
            </View>
            <View>
              <Label>Containers Per Month</Label>
              <Input value={containersPerMonth} onChangeText={setContainersPerMonth} placeholder="Count" keyboardType="numeric" />
            </View>
            <Button onPress={handleSubmit} disabled={submitting || !containerType?.value || !delayType?.value} className="w-full mt-4">
              {submitting ? <ActivityIndicator size="small" color="white" /> : <Text>Submit Pricing Request</Text>}
            </Button>
          </View>
        </View>
      </ScrollView>
