...
export default function WarehousePricingRequestIndexPage() {
  ...
  return (
    <>
      <Stack.Screen
        options={{
          headerTitle: "Warehouse Pricing Request",
          headerLeft: () => (
      ...
      <ScrollView className="flex-1 bg-background">
        <View className="p-4 gap-3">
          <Text className="text-muted-foreground">Select a warehouse provider to submit a pricing request.</Text>
          {providers.length === 0 ? (
            <Card>
              <CardContent className="py-6">
                <Text className="text-muted-foreground text-center">No warehouse providers found.</Text>
              </CardContent>
            </Card>
          ) : (
            providers.map((p) => (
              <Pressable
                key={p.id}
                onPress={() => router.push({ pathname: "/(protected)/warehouse/pricing-request/[providerId]/create", params: { providerId: p.id } })}
              >
                <Card className="active:opacity-90">
                  <CardHeader className="flex-row items-center gap-3">
                    <View className="bg-orange-500/10 rounded-full p-2">
                      <Icon as={Receipt} size={22} className="text-orange-500" />
                    </View>
                    <View className="flex-1">
                      <CardTitle>{p.title || "Provider"}</CardTitle>
                      {p.description && <Text className="text-sm text-muted-foreground mt-0.5" numberOfLines={2}>{p.description}</Text>}
                    </View>
                  </CardHeader>
                </Card>
              </Pressable>
