...
export default function WarehouseOrderEditPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [orderRes, providersRes] = await Promise.all([
        getWarehouseOrderById(orderId),
        getServiceProviders({ serviceTitle: "Warehouse" }),
      ]);
      if (!mounted) return;
      if (orderRes.success && orderRes.output) {
        ...
        setContainersInput(Array.isArray(o.containers) ? (o.containers as string[]).join(", ") : "");
      } else {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await updateWarehouseOrder(orderId, {
  ...
  return (
    <>
      <Stack.Screen
        options={{
          headerTitle: "Edit Warehouse Order",
          headerLeft: () => (
