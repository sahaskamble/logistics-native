...
function getStatusConfig(status?: TransportJobOrderStatus) {
  switch (status) {
    case "Pending":
...
export default function TransportJobOrdersListPage() {
  ...
  const [records] = useState<TransportJobOrderRecord[]>([]);
  const [selectedStatus, setSelectedStatus] = useState<TransportJobOrderStatus | "All">("All");
  ...
  return (
    <>
      <Stack.Screen
        options={{
          headerTitle: "Transport Job Orders",
          headerRight: () => (
            <Button onPress={() => router.push("/(protected)/transport/job-order/create")}>
              <Text>New</Text>
            </Button>
          ),
        }}
      ...
      >
        <View className="p-4 gap-4">
          <Card>
            ...
            <CardContent className="gap-3">
              <View className="gap-2">
                ...
                <Input value={searchQuery} onChangeText={setSearchQuery} placeholder="JOB-... / ORD-..." />
              </View>
          ...
          {filtered.length === 0 ? (
            <Card>
              <CardHeader>
                <CardTitle>No job orders</CardTitle>
              </CardHeader>
              <CardContent>
                <Text className="text-muted-foreground">
                  This screen mirrors the PocketBase collection `transport_job_order`.
                </Text>
              </CardContent>
            </Card>
          ) : (
            filtered.map((r) => (
              <Pressable
                ...
                onPress={() =>
                  router.push({
                    pathname: "/(protected)/transport/job-order/view/[recordId]" as any,
