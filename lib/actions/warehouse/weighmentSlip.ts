...
export function listWarehouseWeighmentSlipRequests(options?: PbQueryOptions) {
  return listWarehouseRequestsByServiceTypeTitle({ serviceTypeTitle: SERVICE_TYPE_TITLE, options });
}

export function getWarehouseWeighmentSlipRequestById(requestId: string, options?: PbQueryOptions) {
  return getWarehouseRequestById({ requestId, options });
}

export function createWarehouseWeighmentSlipRequest(params: {
  orderId: string;
...
export function updateWarehouseWeighmentSlipRequest(params: {
  requestId: string;
  customerRemarks?: string;
